...
def stripe_checkout_view(request):
    ...
    try:
        ...
        return redirect('cart')  # Redirect to cart if no active cart exists

    # Stripe Checkout logic
    cart_total = cart.get_cart_total_price_after_coupon()
    stripe_total = int(cart_total * 100)  # Stripe expects amount in cents
    ...
    session = stripe.checkout.Session.create(
        ...
        cancel_url=request.build_absolute_uri('/accounts/cart/'),
    )
...
def cart(request):
    ...
    try:
        ...
        messages.warning(request, "Your cart is empty. Please add products.")
        return redirect('index')
    ...
    if request.method == 'POST':
        ...
        if cart_obj.get_cart_total() < coupon_obj.minimum_amount:
            messages.warning(request, f'Minimum cart value for this coupon is ₹{coupon_obj.minimum_amount}.')
            return redirect('cart')
    ...
    if cart_obj:
        ...
        if cart_total < 1:
            messages.warning(request, 'Cart total must be at least ₹1. Please add more items.')
            return redirect('index')
        ...
        checkout_session = stripe.checkout.Session.create(
            ...
            mode='payment',
            success_url=domain + 'accounts/success/?session_id={CHECKOUT_SESSION_ID}',
            cancel_url=domain + 'accounts/cancel/',
        )
...
def payment_success_view(request):
    ...
    try:
        ...
        order = Order.objects.create(
            user=user,
            order_id=f'ORD-{session_id}',  # Use session ID as part of order ID
...
def payment_cancel(request):
    messages.warning(request, "Payment canceled. Please try again.")
    return redirect('cart')
